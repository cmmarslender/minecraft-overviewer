...
# get info about owners UID of the directory /home/minecraft/render
MCDIR_OWNER=$(ls -ldn /home/minecraft/render | awk '{print $3}')
...
if [ -n "$SPIGOT_UID" ]; then
  ...
  if [ ! "$MCDIR_OWNER" = "$MC_UID" ]; then
    ...
    echo "Changed owner if minecraft to UID $MCDIR_OWNER"
